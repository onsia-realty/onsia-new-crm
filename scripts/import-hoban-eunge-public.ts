/**
 * 호반은계지구 상담카드 명부 → 공개DB 일괄 등록 ('호반은계지구' 카테고리)
 *
 * 엑셀 구조 (시트: 상담카드_명부):
 *   row 1: "상담 카드 명부" (타이틀)
 *   row 2: 헤더 → No. | 날짜 | 현장명 | 고객명 | 연락처 | 비고
 *   row 3~: 데이터
 *
 * 등록 정책:
 *   - assignedSite = '호반은계지구' (신규 카테고리)
 *   - 엑셀의 행별 현장명(은계 호반/옥길 호반 등)·날짜·비고는 memo 에 보존
 *   - 전화번호 정규화 후 8~11자리만 유효, 엑셀 내 중복 제거
 *   - DB에 이미 존재하는 번호는 분류만 하고 건너뜀(신규만 INSERT)
 *
 * 사용법:
 *   pnpm tsx scripts/import-hoban-eunge-public.ts --dry-run    # 미리보기
 *   pnpm tsx scripts/import-hoban-eunge-public.ts --execute    # 실제 등록
 */
...
const EXCEL_FILE = '호반은계지구_상담카드명부190103.xlsx';
const SITE_NAME = '호반은계지구';
const SITE_COLOR = 'indigo';
const SITE_ICON = '🏞️';
...
async function main() {
  ...
  console.log(`🏷️  카테고리(현장): ${SITE_NAME}`);
  ...
  dataRows.forEach((row, i) => {
    const rowIndex = i + 3; // 엑셀 실제 행번호 (1-based, 타이틀+헤더 2행 보정)
    ...
    if (!normalized || normalized.length < 8 || normalized.length > 11) {
      invalid.push({ rowIndex, rawPhone, reason: `유효하지 않은 자릿수 (${normalized.length}자리)` });
      return;
    }
    if (!normalized.match(/^[0-9]+$/)) {
      invalid.push({ rowIndex, rawPhone, reason: '숫자 외 문자 포함' });
      return;
    }
    ...
    valid.push({
      ...
      name: row[3]?.toString().trim() || null,
      visitDate: excelDateToISO(row[1]),
      subSite: row[2]?.toString().trim() || null,
      remark: row[5]?.toString().trim() || null,
    });
  });
  ...
  if (isDryRun) {
    ...
    console.log(`\n👉 실 등록하려면: pnpm tsx scripts/import-hoban-eunge-public.ts --execute`);
    return;
  }

  // 현장(Site) 카테고리 upsert — 드롭다운/필터에 노출되도록 DB에도 등록
  const maxOrder = await prisma.site.aggregate({ _max: { sortOrder: true } });
  ...
  console.log(`🏷️  현장 카테고리 '${SITE_NAME}' 등록(upsert) 완료`);
  ...
  // displayOrder 시작값 (현재 최소값보다 작게 → 상위 노출)
  ...
  for (let i = 0; i < newRecords.length; i += BATCH_SIZE) {
    ...
    const data = chunk.map((r, idx) => {
      ...
      const subSiteNote = r.subSite ? `[${r.subSite}] ` : '';
      const remark = r.remark ?? '';
      const memo = (visitNote + subSiteNote + remark).trim() || null;
      return {
