/**
 * 웨이브엠 DB 정리 명단 → 공개DB 일괄 등록
 *
 * 엑셀: D:\db\야목역\웨이브엠디비정리.xlsx (헤더 2줄)
 * 엑셀 컬럼: 구분 | 이름 | 전화번호
 *
 * 사용법:
 *   pnpm tsx scripts/import-wavem-public.ts --dry-run    # 미리보기
 *   pnpm tsx scripts/import-wavem-public.ts --execute    # 실제 등록
 */
...
const EXCEL_FILE = 'D:\\db\\야목역\\웨이브엠디비정리.xlsx';
const EXCEL_FILE_LABEL = '웨이브엠디비정리.xlsx';
const SITE_NAME = '웨이브엠';
...
async function main() {
  ...
  if (isDryRun) {
    ...
    console.log(`\n👉 실 등록하려면: pnpm tsx scripts/import-wavem-public.ts --execute`);
    return;
  }
