/**
 * 민간임대 파비안.xlsx → 공개DB 일괄 등록 ('민간임대 3억대' 카테고리)
 *
 * 엑셀 구조 (시트: Sheet1, 헤더 없음 — 1행부터 데이터):
 *   col A(0): 순번  | col B(1): 고객명 | col C(2): 연락처
 *   연락처는 8자리(010 생략, 예 "6330-3006") 또는 11자리("010-...") 혼재
 *
 * 등록 정책:
 *   - assignedSite = '민간임대 3억대' (신규 카테고리)
 *   - normalizePhone 으로 8자리→010 보정, 8~11자리만 유효
 *   - 엑셀 내 중복(정규화 동일번호) 제거 — 첫 등장만 keep
 *   - DB에 이미 존재하는 번호는 분류만 하고 건너뜀(신규만 INSERT)
 *   - memo 없음(원본에 비고/날짜 컬럼 없음)
 *
 * 사용법:
 *   pnpm tsx scripts/import-fabian-public.ts --dry-run    # 미리보기
 *   pnpm tsx scripts/import-fabian-public.ts --execute    # 실제 등록
 */
...
const EXCEL_FILE = '민간임대 파비안.xlsx';
const SITE_NAME = '민간임대 3억대';
const SITE_COLOR = 'emerald';
const SITE_ICON = '🏘️';
...
async function main() {
  ...
  dataRows.forEach((row, i) => {
    const rowIndex = i + 1; // 엑셀 실제 행번호 (1-based, 헤더 없음)
  ...
  if (isDryRun) {
    ...
    console.log(`\n👉 실 등록하려면: pnpm tsx scripts/import-fabian-public.ts --execute`);
    return;
  }
