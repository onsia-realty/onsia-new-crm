/**
 * 안산서희스타힐스6차 명단 → 공개DB 일괄 등록
 *
 * 엑셀: D:\db\야목역\안산서희스타힐스6차.xlsx (헤더 2줄)
 * 엑셀 컬럼: 구분 | 이름 | 전화번호
 *
 * 사용법:
 *   pnpm tsx scripts/import-ansan-seohee6-public.ts --dry-run    # 미리보기
 *   pnpm tsx scripts/import-ansan-seohee6-public.ts --execute    # 실제 등록
 */
...
const EXCEL_FILE = 'D:\\db\\야목역\\안산서희스타힐스6차.xlsx';
const EXCEL_FILE_LABEL = '안산서희스타힐스6차.xlsx';
const SITE_NAME = '안산서희스타힐스6차';
...
async function main() {
  ...
  const rows = XLSX.utils.sheet_to_json<ExcelCell[]>(sheet, { header: 1, raw: false, blankrows: false });
  ...
  dataRows.forEach((row, i) => {
    ...
    if (!rawPhone) {
      invalid.push({ rowIndex: i + 3, rawPhone: '', reason: '연락처 없음' });
      return;
    }
    const normalized = normalizePhone(rawPhone);
    if (!normalized || normalized.length < 8 || normalized.length > 11) {
      invalid.push({ rowIndex: i + 3, rawPhone, reason: `유효하지 않은 자릿수 (${normalized.length}자리)` });
      return;
    }
    if (!normalized.match(/^[0-9]+$/)) {
      invalid.push({ rowIndex: i + 3, rawPhone, reason: '숫자 외 문자 포함' });
      return;
    }
  ...
  if (isDryRun) {
    ...
    console.log(`\n👉 실 등록하려면: pnpm tsx scripts/import-ansan-seohee6-public.ts --execute`);
    return;
  }
