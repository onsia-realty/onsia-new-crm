/**
 * 안산 중앙역 아피체 방명록 → 공개DB 등록 (현장: 중앙역 아피체)
 * 사용: node scripts/register_ansan_apiche.js            (dry-run)
 *       node scripts/register_ansan_apiche.js --execute  (실제 등록)
 *
 * 입력: scripts/ansan_apiche.txt (탭 구분: 이름 / 거주지역 / 번호숫자)
  ...
  try {
    const raw = fs
      ...
      .map((l) => {
        ...
        return { name: (name || '').trim(), area: (area || '').trim(), phone: (phone || '').trim() };
      });
    ...
    await prisma.auditLog.create({
      data: {
        ...
        entity: 'Customer',
        changes: {
          source: '안산 중앙역 아피체 방명록 (중복 수정).xlsx',
          ...
          assignedSite: SITE,
          isPublic: true,
          mode: 'ansan-apiche-guestbook-bulk-script',
        },
      },
    });
